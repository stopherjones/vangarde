...
const InteractiveDie: React.FC<{
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative flex items-center gap-1.5 p-1.5 rounded-lg border-2 transition-all cursor-pointer select-none text-left ${
        isDirectionChosen
    ...
    >
      {/* 3x3 Pip Dice Box */}
      <div
        className={`w-11 h-11 rounded-md border-2 grid grid-cols-3 grid-rows-3 p-1 shrink-0 ${
          isDirectionChosen
      ...
      <div className="flex flex-col min-w-[70px] pr-0.5">
        <span
        ...
        <span className="text-sm font-mono font-black leading-tight flex items-center gap-1">
        ...
        <span
          className={`text-[8.5px] font-mono underline opacity-80 group-hover:opacity-100 ${
        ...
        {!diceState.rolled ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={energy <= 0}
              className={`py-2 px-2.5 text-xs font-mono font-bold rounded-lg border-2 border-[#2b261f] flex items-center gap-1.5 transition-colors cursor-pointer shrink-0 ${
                isMoveOne
            ...
            >
              ...
              <span>{isMoveOne ? 'Cancel' : 'Move 1 (-1⚡)'}</span>
            </button>
          </div>
        ) : (
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center justify-between gap-1 flex-wrap sm:flex-nowrap">
              ...
              {/* Action Buttons: Reset Deviation and Move 1 step option */}
              <div className="flex items-center gap-1 shrink-0 ml-auto">
                {deviationState.active && onResetDeviation && (
                  <button
                    onClick={onResetDeviation}
                    className="py-1 px-1.5 text-[10px] font-mono font-bold rounded border border-[#2b261f] bg-[#fef3c7] hover:bg-[#fde68a] text-[#b45309] flex items-center gap-1 cursor-pointer"
                ...
                <button
                  ...
                  onClick={onToggleMoveOne}
                  disabled={energy <= 0}
                  className={`py-1.5 px-2 text-[11px] font-mono font-bold rounded-md border-2 border-[#2b261f] flex items-center gap-1 transition-all cursor-pointer ${
                    isMoveOne
                      ? 'bg-[#d97706] text-white ring-2 ring-[#92400e]'
                      : 'bg-[#f5efe3] hover:bg-[#fff9ed] text-[#2b261f]'
                  }`}
            ...
            {deviationState.active && deviationState.type === 'split_path' ? (
              <div className="flex items-center justify-between px-2 py-0.5 bg-[#fef3c7] border border-[#d97706] rounded text-[10px] font-mono text-[#92400e]">
                <div className="flex items-center gap-1">
                  <CornerDownRight className="w-3 h-3 text-[#d97706]" />
                  <span>
